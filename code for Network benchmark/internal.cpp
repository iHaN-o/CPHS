...
void Internal::allocmemory() {
    ...
    score = new ll[opts.poolsize + 1];
    block = new vec<int>[nnum + 1];
    son = new int[nnum + 1];
    cutson = new int[nnum + 1];
    cutscore = new ll[nnum + 1];
...
void Internal::readlargefile(char *file) {
    printf("c read large file\n");
    freopen(file, "r", stdin);
    nnum = getint();
    mnum = getint();
    knum = nnum / opts.knum;
...
void Internal::tarjan(int u, int fa, int rt) {
    ...
    for (int i = 0; i < edge[u].size(); i++) {
        ...
        nson++;
        if (u != rt && low[v] >= dfn[u]) is = 1, cutscore[u] += 1ll * son[v] * (son[v] - 1) / 2, cutson[u] += son[v]; // add
        if (u == rt) cutscore[u] += 1ll * son[v] * (son[v] - 1) / 2, cutson[u] += son[v];
    }
...
int Internal::findcut(int pick) {
    ...
    for (int i = 0; i < block[pick].size(); i++) {
        int u = block[pick][i];
        cutscore[u] += 1ll * (block[pick].size() - cutson[u]) * (block[pick].size() - cutson[u] - 1) / 2;
...
void Internal::deletenode(int num) {
    ...
    // ll prescore = 0, aftscore = 0;
    // for (int i = 1; i <= nblock; i++)
    //     prescore += 1ll * block[i].size() * (block[i].size() - 1) / 2;
    ll mnval = 1e18;
    int delp = 0, pos = 0;
    for (int i = 1; i <= num - deleteFlag; i++) {
        ...
        for (int j = 0; j < edge[x].size(); j++) {
            ...
            delta -= 1ll * block[v].size() * (block[v].size() - 1) / 2;
        }
        delta += 1ll * comb * (comb - 1) / 2;
    ...
    // for (int i = 1; i <= nblock; i++)
    //     aftscore += 1ll * block[i].size() * (block[i].size() - 1) / 2;
...
ll Internal::improve() {
    ...
    for (int i = 1; i <= nblock; i++)
        round_best += 1ll * block[i].size() * (block[i].size() - 1) / 2;
    ...
    while (++nowIter <= rounditer + opts.nonimprove) {
        ...
        ll nowscore = 0;
        for (int i = 1; i <= nblock; i++)
            nowscore += 1ll * block[i].size() * (block[i].size() - 1) / 2;
        //printf("c %d\n", nowscore);
        if (nowscore < round_best) {
            if (nowscore < best_val) {
                printf("c improve: %lld\n", round_best);
            }
            // printf("c Round %d improve: %lld\n", nowIter, nowscore);
            round_best = nowscore, rounditer = nowIter;
            for (int i = 1; i <= knum; i++) round_node[i] = sol[i];
        }
        auto clk_now = std::chrono::high_resolution_clock::now();
        int caltime = std::chrono::duration_cast<std::chrono::seconds>(clk_now - clk_st).count();
        if (caltime > opts.times) break;
    }
...
//  while (std::chrono::duration_cast<std::chrono::milliseconds>(clk_now - clk_st).count() < opts.times) {
    ...
    printf("c final best val(simpsolve): %lld result, find in %lu ms\n\n\n", best_val,
...
void Internal::solve() {
    ...
    printf("c finish initial, now val: %lld\n", best_val);
    do {
        if (caltime > opts.times) break;
        if (best_val <= opts.bestres) break;
    ...
    printf("c final best val(solve): %lld result, find in %lu ms\n", best_val,
           std::chrono::duration_cast<std::chrono::milliseconds>(best_time - clk_st).count());
    printf("c solution: ");
    for (int i = 1; i <= knum; i++)
